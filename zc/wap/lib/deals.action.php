...
class dealsModule{
	...
	{	
       ...
       $city_list = load_dynamic_cache("INDEX_CITY_LIST"); 
         if(!$city_list)
		{
			$city_list = $GLOBALS['db']->getAll("select d.province,rc.id from ".DB_PREFIX."deal as d left join ".DB_PREFIX."region_conf as rc on d.province=rc.name  group by d.province order by d.sort asc");
		...
		switch($state)
		{
			//筹资成功
			case 1 : 
				$condition.=" and (select sum(di.virtual_person*di.price+di.support_amount) from ".DB_PREFIX."deal_item di where di.deal_id=d.id)  >= d.limit_price  and d.end_time!=0  and d.end_time<".NOW_TIME." "; 
				$GLOBALS['tmpl']->assign("page_title","筹资成功");
				break;
			//筹资失败
			case 2 : 
				$condition.=" and d.end_time < ".NOW_TIME." and d.end_time!=0  and ((select sum(di.virtual_person*di.price+di.support_amount) from ".DB_PREFIX."deal_item di where di.deal_id=d.id)  < d.limit_price or (select count(*) from ".DB_PREFIX."deal_item di_1 where di_1.deal_id=d.id)=0 )  "; 
