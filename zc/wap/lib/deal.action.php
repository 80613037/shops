...
class dealModule{
	...
	public function index(){
        ...
        $deal_info['begin_date'] =date('Y/m/d',$deal_info['begin_time']);
        $deal_info['end_date'] =date('Y/m/d',$deal_info['end_time']);
        if($deal_info['user_id'] == 0){
            $deal_info['shopName'] = app_conf("SITE_NAME");
            $deal_info['site_logo'] = app_conf("SITE_LOGO");
        }else{
            $sql = "SELECT fu.userName,fu.userPhoto FROM fanwe_user fwu LEFT JOIN fys_users fu ON fwu.user_name=fu.loginName WHERE fwu.id=".$deal_info['user_id'];
            $shopInfo = $GLOBALS['db']->getAll($sql);
//            var_dump($shopInfo);
            $deal_info['shopName'] = $shopInfo[0]['userName'];
            $deal_info['site_logo'] = get_domain().'/'.$shopInfo[0]['userPhoto'];
        }
		if(!$deal_info)
		{
...
//		    print_r($deal_info);exit;
	...
	public function comment(){
		...
		$comment_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal_comment where deal_id = ".$id." and log_id = 0 order by create_time asc");
	
		$comment_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal_comment where deal_id = ".$id." and log_id = 0");
		
		$comment_list[1]['save_url_wap']=url_wap("deal#savedealcomment");
	...
	public function save_update()
	{
		...
		else
		{
			...
			showSuccess("",$ajax,url_wap("deal#update",array("id"=>$deal_info['id'])));
		}
	}
