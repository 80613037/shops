...
//		}
	...
	*/	
	...
	{		
		...
		if($field_name=='verify_coder'&&$user_data['verify_coder']!=''&&$GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."mobile_verify_code WHERE mobile=".strim($_REQUEST['mobile'])." AND verify_code='".trim($user_data['verify_coder'])."'")==0)
		{
 			$field_item['field_name'] = 'verify_coder';
			$field_item['error']	=	EXIST_ERROR;
			$res['status'] = 0;
