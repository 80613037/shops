...
function send_user_verify_mail($user_id)
{
	if(app_conf("MAIL_ON")==1)
	{
		...
		if($user_info)
		{
			...
			$user_info['verify_url'] = get_domain().url("user#verify",array("id"=>$user_info['id'],"code"=>$user_info['verify']));
...
//发修改邮箱验证邮件第一步
function send_user_verify_mail_change($user_id)
{
	if(app_conf("MAIL_ON")==1)
	{
		$user_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where id = ".$user_id);
		if((get_gmtime()-$user_info['verify_time'])>48*3600){
			$verify_code = rand(111111,999999);
 			$GLOBALS['db']->query("update ".DB_PREFIX."user set verify = '".$verify_code."',verify_time=".get_gmtime()." where id = ".$user_id);
 		}

		if($user_info)
		{
			$tmpl = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."msg_template where name = 'TPL_MAIL_CHANGE_USER_VERIFY'");
			$tmpl_content=  $tmpl['content'];
			...
			$user_info['verify_url'] = get_domain().url("settings#mail_change_verify",array("id"=>$user_info['id'],"code"=>$user_info['verify'],"step"=>1));
			...
			$msg_data['dest'] = $user_info['email'];
			$msg_data['send_type'] = 1;
			$msg_data['title'] = app_conf("SITE_NAME")."帐号-邮箱修改验证";
			$msg_data['content'] = addslashes($msg);;
			$msg_data['send_time'] = 0;
			$msg_data['is_send'] = 0;
			$msg_data['create_time'] = get_gmtime();
...
//发修改邮箱验证邮件第二步
function send_user_verify_mail_setting($user_id,$email)
{
	if(app_conf("MAIL_ON")==1)
	{
		$user_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where id = ".$user_id);
		if((get_gmtime()-$user_info['verify_setting_time'])>48*3600){
					$verify_code = rand(111111,999999);
					$GLOBALS['db']->query("update ".DB_PREFIX."user set verify_setting = '".$verify_code."',verify_setting_time=".get_gmtime()." where id = ".$user_id);
		...
		if($user_info)
		{
			...
			$user_info['logo']=app_conf("SITE_LOGO");
			$user_info['site_name']=app_conf("SITE_NAME");
			$user_info['zc_type']="设置";
			$time=get_gmtime();
			$user_info['send_time']=to_date($time,'Y年m月d日');
			$user_info['send_time_ms']=to_date($time,'Y年m月d日 H时i分');

			$user_info['verify_url'] = get_domain().url("settings#mail_change_verify",array("id"=>$user_info['id'],"code"=>$user_info['verify_setting'],"step"=>2,"e"=>base64_encode($email)));
			...
			$msg_data['dest'] = $user_info['email'];
			$msg_data['send_type'] = 1;
			$msg_data['title'] = app_conf("SITE_NAME")."帐号-邮箱设置";
			$msg_data['content'] = addslashes($msg);;
			$msg_data['send_time'] = 0;
			$msg_data['is_send'] = 0;
			$msg_data['create_time'] = get_gmtime();
...
//发注册验证邮件
function send_wap_user_verify_mail($user_id)
{
	if(app_conf("MAIL_ON")==1)
	{
		$verify_code = rand(111111,999999);
		...
		if($user_info)
		{
			...
			$user_info['verify_url'] = get_domain().(url_wap("user#verify")."&id=".$user_info['id']."&code=".$user_info['verify']);
...
//发邮件验证码
function send_verify_email($email,$code,$type="nomal")
{
	if(app_conf("MAIL_ON")==1)
	{
 		$tmpl = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."msg_template where name = 'TPL_MAIL_USER_VERIFY'");
			...
			$msg_data['dest'] =$email;
			$msg_data['send_type'] = 1;
			$msg_data['title'] = app_conf("SITE_NAME")."验证码";
			$msg_data['content'] = addslashes($msg);;
			$msg_data['send_time'] = 0;
			$msg_data['is_send'] = 0;
			$msg_data['create_time'] = get_gmtime();
...
function send_investor_status($user_info){
	$invest_status=app_conf("INVEST_STATUS_SEND_STATUS");
	$status=intval($user_info['investor_status']);
	...
	$user_info['investor_status_name']=get_investor_status($user_info['investor_status']);
 	if(!empty($user_info['email'])&&$invest_status==1){
 		$tmpl = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."msg_template where name = 'TPL_MAIL_INVESTOR_STATUS'");
			$tmpl_content=  $tmpl['content'];
			$user_info['logo']=app_conf("SITE_LOGO");
			$user_info['site_name']=app_conf("SITE_NAME");
			$time=get_gmtime();
			...
			$re=$GLOBALS['db']->autoExecute(DB_PREFIX."deal_msg_list",$msg_data); //插入
 			...
 			send_notify($user_info['id'],$info,$info_url,"");
 			return $re;
	}elseif(!empty($user_info['mobile'])&&$invest_status==2){
 				$tmpl = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."msg_template where name = 'TPL_SMS_INVESTOR_STATUS'");
				$tmpl_content = $tmpl['content'];
				$verify['mobile'] = $user_info['mobile'];
 				$GLOBALS['tmpl']->assign("verify",$verify);
 				$GLOBALS['tmpl']->assign("user",$user_info);
				$msg = $GLOBALS['tmpl']->fetch("str:".$tmpl_content);
...
function get_investor($is_investor){
 ...
 function investor_agency_save($id,$ajax='',$identify_business_name,$identify_business_licence,$identify_business_code,$identify_business_tax,$image1,$image2,$image3)
 {
 	...
 	if($GLOBALS['db']->query("update ".DB_PREFIX."user set identify_business_name='".$identify_business_name."',identify_business_licence='".$image1."',identify_business_code='".$image2."',identify_business_tax='".$image3."',investor_status=0 where id=".$id)>0){
 		$data['status'] = 1;
 		$data['info'] = "认证信息入录成功！";
