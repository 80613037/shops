...
class userModule extends BaseModule
{
	...
	public function do_login(){
	    file_put_contents('L34.sql',json_encode($_POST));
		if(!$_POST)
		{
    ...
    public function do_register(){
        file_put_contents('zclog.log','737'.PHP_EOL, FILE_APPEND);
			$email = strim($_REQUEST['email']);
        file_put_contents('zclog.log','739'.$email.PHP_EOL, FILE_APPEND);
			require_once APP_ROOT_PATH."system/libs/user.php";
			$return = $this->register_check_all();
        file_put_contents('zclog.log','742'.json_encode($return).PHP_EOL, FILE_APPEND);
			if($return['status']==0)
			{
				ajax_return($return);
			}		
			$user_data = $_POST;
        file_put_contents('zclog.log','748'.json_encode($user_data).PHP_EOL, FILE_APPEND);
			foreach($_POST as $k=>$v)
			{
				$user_data[$k] = strim($v);
			}	
