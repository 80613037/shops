 ...
 * deal 表数据实体处理
 */
function formateDealInfo($deal_id, $user_id)
{
 ...
 * 领投信息实体处理
 */
function formateLeader_info($leader_info)
{
	$leader_info ['money'] = $leader_info ['money'];
...
function formateType2DealInfo($deal_info)
{
	...
	$deal_info_type2 ['invote_money'] = doubleval ( $deal_info ['invote_money'] );
...
function formateType1DealInfo($deal_info)
{
	...
	// status值0表示即将开始；1表示已成功；2表示筹资失败；3表示筹资中；4表示长期项目
	// 剩余时间remain_days
 ...
 * 成功时候返回信息
 */
function responseSuccessInfo($successInfo)
{
	$result ['response_code'] = 1;
...
function getLeaderInfo($deal_id)
{
	...
	if ($leader_info > 0)
	{
		$leader_info = formateLeader_info ( $leader_info );
	}
	return $leader_info;
}
 ...
 * $is_only 是否必须验证手机唯一性
 */
 function send_code_function($mobile,$is_only, $email, $pwd)
{
