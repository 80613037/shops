...
//        WLog('log_money', 'Mark',  M('log_moneys')->getLastSql());
     ...
     * $stat 0 未发货；1已发货
     */
    public function exchangeRecord($stat, $limit){
        if($stat == 1){
            $where = " AND o.orderStatus in (1,2) ";
            $sql = "SELECT o.orderId,og.goodsName,og.exchangeScore,og.goodsThums,og.goodsNums FROM __PREFIX__orders o LEFT JOIN __PREFIX__order_goods og ON o.orderId = og.orderId WHERE o.payType=9 ". $where." ORDER BY o.orderId DESC limit ".$limit;
        }else{
            $sql = "SELECT o.orderId,og.goodsName,og.exchangeScore,og.goodsThums,og.goodsNums FROM __PREFIX__orders o LEFT JOIN __PREFIX__order_goods og ON o.orderId = og.orderId WHERE o.payType=9 AND o.orderStatus=".$stat." ORDER BY o.orderId DESC limit ".$limit;
        }
        $data = $this->query($sql);
        return $data;
    }
