...
function checkLoginInfo(){
	...
	$.post('/index.php?m=Home&c=Users&a=checkLogin',{loginName:loginName,loginPwd:loginPwd,from:from},function(data,textStatus){
		...
		if(json.status=='1'){
			// 模拟 众筹登陆
            $.post('/zc/wap/index.php?ctl=user&act=do_login',{email:loginName,user_pwd:loginPwd,ajax:1,auto_login:0},function(data,textStatus){});
            layer.msg("登录成功！");
...
function logout(){
    jQuery.post(('index.php?m=Home&c=Users&a=logout'),{},function(rsp) {
        layer.msg("成功退出！");
        jQuery.get(('zc/wap/index.php?ctl=user&act=loginout'),{},function(rsp){});
