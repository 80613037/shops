...
class UsersAction extends BaseAction {
	...
	public function logout(){
        ...
        setcookie("user_pwd",null);
        setcookie("hide_user_notify",null);
	 ...
	 * 模拟众筹登陆
	 */
	public function zclogin(){
        $uri = "http://192.168.1.166/zc/wap/index.php?ctl=user&act=do_login"; // 接口地址
        // 参数数组
        $data = array (
            'ajax' => '1',
            'auto_login'=>0,
            'email' => '18932900603', // I('loginName'),
            'user_pwd' => 'admin123'  // I('loginPwd')
        );
...
//    				$this->display('default/forget_pass2');
    ...
    public function exchangeRecord(){
        $um = D("Home/Users");
        $glistAlreadyshipped = $um->exchangeRecord(1, 20); // 未发货
        $glistNotShipped = $um->exchangeRecord(3, 20);     // 已发货
